...
export interface AccountInfo { account: AccountAddress, address: string, ansName?: string }
...
interface IWallet {
  connect: (silent?: boolean, network?: Network) => Promise<AccountInfo>;
...
const PetraWallet = () => {
  ...
  if(!wallet) {
    window.open('https://chromewebstore.google.com/detail/petra-aptos-wallet/ejjladinnckdgjemekebdpeokbikhfci?hl=zh-CN&utm_source=ext_sidebar', '_blank')
  }
  ...
  const connectWallet = async () => {
    ...
    if(wallet){
      ...
      if(connectRes.address) {
        setAccountAddress(connectRes.address)
      }
      // if(connectRes.status === UserResponseStatus.APPROVED) {
      //   setAccountAddress(connectRes.args.address)
      // }
    }
  }
  ...
  return (
    <Box>
      <h1>Test Petra Wallet</h1>
      <Box>
        Is the wallet ready: { wallet ? 'true' : 'false' } (window.aptos)
      </Box>
      <Box
        sx={{
          margin: '20px 12px'
