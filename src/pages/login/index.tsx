...
import { Network, Account, Signature, AnyRawTransaction, InputGenerateTransactionPayloadData, EndlessConfig, Endless, AccountAddress } from 'endless-ts-sdk'
...
const LoginPage = () => {
  ...
  const init = async () => {
    if('endWallet' in window) {
      const wallet = window.endWallet

      const accountInfo = await wallet.connect(true, Network.TESTNET) // connect to testnet
      console.log('accountInfo =====> ',accountInfo)

      const getAccount = await wallet.getAccount()
      console.log('getAccount =====> ',getAccount)

      const getNetwork = await wallet.getNetwork()
      ...
      const address = getAccount.status === UserResponseStatus.APPROVED ? getAccount.args.account : ''
      ...
      const transaction = await endless.transaction.build.simple({
        sender: AccountAddress.fromBs58String(address as string),
        data: {
      ...
      const signTransaction = await wallet.signTransaction(transaction) // sign a transaction
      console.log('signTransaction =====> ',signTransaction)

      const signAndSubmitTransaction = await wallet.signAndSubmitTransaction({payload:{
        function: '0x1::endless_account::transfer',
        functionArguments: [
          AccountAddress.fromBs58String(toAddress),
          BigInt(amount*100000000),
      ...
      const changeNetwork = await wallet.changeNetwork({
        name: Network.DEVNET,
        chainId: 4
      }) // change network
